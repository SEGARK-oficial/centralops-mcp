    ...
    return [
        ToolSpec(
            ...
            description=(
                ...
                "generated_at, kpis, and top_buckets. kpis is a list of cards "
                ...
                "('healthy/total'), active_sources — plus total_alerts only when "
                "alerts exist in the window. top_buckets is a list of ranking "
                ...
                "top_route_drops — plus alerts_by_severity only when alerts exist. "
                "Labels are localized (pt-BR). Results are scoped to organizations "
                "the token's user can access; passing organization_id outside that "
                "subtree returns 403, an unknown integration_id returns 404, and an "
                "integration_id that mismatches organization_id or platform returns "
                "409. An unknown platform is not rejected — it just yields empty "
                "data. Use this as the first read in a triage session before "
                "drilling into collectors, quarantine, or destinations."
            ),
